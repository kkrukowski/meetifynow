...
export default function AnswerMeeting({
    ...
    for (let i = timeRange.minimumTimeHour; i <= timeRange.maximumTimeHour; i++) {
      for (let h = 0; h < 2; h++) {
        ...
        for (const date of staticMeetingData.dates) {
          ...
          const usersInfoCount = availabilityInfo[dateTime]?.usersInfo.length || 0;
          if (usersInfoCount > highestAvailableCount) {
            setHighestAvailableCount(usersInfoCount);
          }
  ...
  }, [isSendingReq, username, selectedTimecells, staticMeetingData.appointmentId, addAnswer]);
